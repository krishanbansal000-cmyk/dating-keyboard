...
enum class ToolbarKey {
    ...
    PAGE_UP, PAGE_DOWN, FULL_LEFT, FULL_RIGHT, PAGE_START, PAGE_END, RIZZSE
}
...
val defaultToolbarPref by lazy {
    val default = listOf(SETTINGS, VOICE, CLIPBOARD, UNDO, REDO, SELECT_WORD, COPY, PASTE, LEFT, RIGHT, RIZZSE)
...
val defaultPinnedToolbarPref = entries.filterNot { it == CLOSE_HISTORY }.joinToString(Separators.ENTRY) {
    it.name + Separators.KV + (it == RIZZSE)
}
...
private fun upgradeToolbarPref(prefs: SharedPreferences, pref: String, default: String) {
    ...
    splitDefault.forEach { entry ->
        ...
        if (list.none { it.startsWith(keyWithSeparator) })
            list.add(entry)
        else if (pref == Settings.PREF_PINNED_TOOLBAR_KEYS && entry.startsWith(RIZZSE.name + Separators.KV)) {
            val idx = list.indexOfFirst { it.startsWith(keyWithSeparator) }
...
private fun getEnabledToolbarKeys(prefs: SharedPreferences, pref: String, default: String): List<ToolbarKey> {
    ...
    val enabledKeys = string.split(Separators.ENTRY).mapNotNull {
        val split = it.split(Separators.KV)
        if (split.last() == "true" || (pref == Settings.PREF_PINNED_TOOLBAR_KEYS && split.first() == RIZZSE.name)) {
            try {
    ...
    return if (pref == Settings.PREF_PINNED_TOOLBAR_KEYS && enabledKeys.contains(RIZZSE))
        listOf(RIZZSE) + enabledKeys.filterNot { it == RIZZSE }
    else
        enabledKeys
}
