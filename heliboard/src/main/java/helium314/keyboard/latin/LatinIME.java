...
public class LatinIME extends InputMethodService implements
    ...
    void onStartInputViewInternal(final EditorInfo editorInfo, final boolean restarting) {
        ...
        if (mInputView != null) {
            mInputView.postDelayed(this::showPendingRizzseSuggestions, 500);
        } else {
    ...
    void launchSettings() {
        ...
        intent.setClass(LatinIME.this, helium314.keyboard.settings.SettingsActivity2.class);
    ...
    public void sendRizzseAction(String action) {
        try {
            String textBeforeCursor = "";
            try {
                final var ic = getCurrentInputConnection();
                if (ic != null) {
                    CharSequence cs = ic.getTextBeforeCursor(500, 0);
                    if (cs != null) textBeforeCursor = cs.toString();
                }
            } catch (Exception ignored) { }
            requestHideSelf(0);
            android.content.SharedPreferences prefs = getSharedPreferences("dating_copilot", Context.MODE_MULTI_PROCESS);
            prefs.edit()
                .putString("rizzse_pending_action", action)
                .putString("rizzse_chat_context", textBeforeCursor)
                .apply();
            android.content.Intent intent = new android.content.Intent("com.datingcopilot.keyboard.RIZZSE_ACTION");
            intent.putExtra("action", action);
            intent.putExtra("chat_context", textBeforeCursor);
            intent.setPackage("com.datingcopilot.keyboard");
            sendBroadcast(intent);
        } catch (Exception e) {
            Log.w("LatinIME", "sendRizzseAction failed: " + e.getMessage());
        }
    }
