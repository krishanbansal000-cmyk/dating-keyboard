...
class SuggestionStripView(context: Context, attrs: AttributeSet?, defStyle: Int) :
    ...
    init {
        ...
        if (!isGone && !Settings.getValues().mSuggestionStripHiddenPerUserSettings) {
            ...
            for (pinnedKey in getPinnedToolbarKeys(context.prefs())) {
                if (pinnedKey == ToolbarKey.RIZZSE) continue
    ...
    private fun createRizzseVibeButton(): LinearLayout {
        val prefs = context.getSharedPreferences("dating_copilot", Context.MODE_PRIVATE)
        val currentVibe = prefs.getString("persona", "playful") ?: "playful"
    ...
    private fun showVibePopup(anchor: LinearLayout) {
        val popup = android.widget.PopupWindow(context)
        val vibes = listOf(
            "playful" to "Rizz",
            "witty" to "Funny",
            "chill" to "Chill",
        ...
        val currentPersona = context.getSharedPreferences("dating_copilot", Context.MODE_PRIVATE)
        ...
        val layout = LinearLayout(context).apply {
            ...
            background = GradientDrawable().apply {
                cornerRadius = dp(16).toFloat()
                setColor(0xFF2B1535.toInt())
                setStroke(dp(1), 0xFF4A255E.toInt())
            }
            elevation = dp(8).toFloat()
        }

        vibes.forEach { (persona, label) ->
            ...
            layout.addView(TextView(context).apply {
                ...
                setTextColor(if (isSelected) 0xFFFF38F8.toInt() else 0xFFEBD5FF.toInt())
                setPadding(dp(12), dp(10), dp(12), dp(10))
                gravity = Gravity.CENTER_VERTICAL
                isClickable = true
                setOnClickListener {
                    context.getSharedPreferences("dating_copilot", Context.MODE_PRIVATE)
                        .edit()
                        .putString("persona", persona)
                        .putString("intent", when (persona) {
                            "playful" -> "flirt"
                            "witty" -> "recover_dry"
                            "chill" -> "keep_going"
                            "direct" -> "ask_date"
                        ...
                        .apply()
                    // Find the TextView inside the anchor container and update it
                    ...
                    anchor.contentDescription = "RizzSe vibe: $label"
                    popup.dismiss()
                }
            })
        }
    ...
    private fun createRizzseRecordButton(): LinearLayout {
        val container = LinearLayout(context).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER
            background = GradientDrawable().apply {
    ...
    private fun createRizzseMagicButton(): LinearLayout {
        val container = LinearLayout(context).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER
            background = GradientDrawable().apply {
