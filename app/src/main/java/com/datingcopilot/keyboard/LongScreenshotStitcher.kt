...
class LongScreenshotStitcher {
    ...
    companion object {
        ...
        private const val ROW_SAMPLE_STEP = 4
    ...
    fun stitchFast(frames: List<Bitmap>): Bitmap? {
        ...
        for (i in 1 until frames.size) {
            ...
            for (shift in minShift..maxShift step (ROW_SAMPLE_STEP * 4)) {
                ...
                for (row in 0 until overlap step stepRows) {
                    ...
                    totalScore += colScore / colXs.size
                    compared++
                }
                if (compared > 0) {
                    val avg = totalScore / compared
                    if (avg < bestScore) { bestScore = avg; bestShift = shift }
                }
            }
